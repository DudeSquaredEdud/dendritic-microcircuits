...
logger = get_logger("ai.experiments.ApicalConvergenceTest")
...
class ApicalConvergenceTest(Experiment):

    def __init__(self, params: MultiDict):
        # call the constructor of the parent class (aka superclass)
        super().__init__(params)

        self.visual_extractor = VisualExtractor("ACT")

        self._nudge1 = params.get("nudge1", 1.0, type=float)
        self._nudge2 = params.get("nudge2", 0.0, type=float)
        ...
        self.__self_prediction_steps = params.get(
            "self_prediction_steps", 400, type=int
        )
        ...
        self.build_small_three_layer_network(
            self.__n_pyr_layer1, self.__n_pyr_layer2, self.__n_pyr_layer3
        )
    ...
    def __do_ff_sweep(self):
        ...
        for prev, layer in iter_with_prev(
            self.layers
        ):  # Yields prev layer and current layer
            if prev is None:
                layer.apply_inputs_to_test_self_predictive_convergence([0.5, 0.5])
            else:
    ...
    def __train_1_step_rule_16b_and_rule_13(self, use_nudge=False, use_rule_ip=False):
        ...
        # Visual Extractor
        self.visual_extractor.layers_to_file([l1, l2, l3])
    ...
    def __nudge_output_layer(self):
        ...
        last_layer = self.layers[-1]
        last_layer.nudge_output_layer_neurons(
            self._nudge1, self._nudge2, lambda_nudge=0.9
        )
        ...
        logger.info(
            "Finished 1st FB sweep after nudge: pilot_exp_2b"
    ...
    def _hook_post_train_step(self):
        ...
        self._metrics[KEY_RULE_13_POST_DATA] = np.append(
            ...
            create_column_vector(
                soma_act, basal_hat_act, post_soma_mp, post_basal_mp, post_val
            ),
            axis=1,
        )
        ...
        self._metrics[KEY_RULE_13_WT_DATA_L2_PYR0] = np.append(
            ...
            create_column_vector(
                l3.pyrs[0].W_PP_ff[0], l3.pyrs[0].W_PP_ff[1], l3.pyrs[0].W_PP_ff[2]
            ),
            axis=1,
        )
        ...
        self._metrics[KEY_RULE_13_WT_DATA_L2_PYR1] = np.append(
            ...
            create_column_vector(
                l3.pyrs[1].W_PP_ff[0], l3.pyrs[1].W_PP_ff[1], l3.pyrs[1].W_PP_ff[2]
            ),
            axis=1,
        )
        ...
        self._metrics[KEY_RULE_16B_WT_DATA_L2_PYR0] = np.append(
            ...
            create_column_vector(
                l2.pyrs[0].W_PI_lat[0], l2.pyrs[0].W_PI_lat[1], l2.pyrs[0].W_PI_lat[2]
            ),
            axis=1,
        )
        ...
        self._metrics[KEY_RULE_16B_WT_DATA_L2_PYR1] = np.append(
            ...
            create_column_vector(
                l2.pyrs[1].W_PI_lat[0], l2.pyrs[1].W_PI_lat[1], l2.pyrs[1].W_PI_lat[2]
            ),
            axis=1,
        )
        ...
        self._metrics[KEY_RULE_16B_WT_DATA_L2_PYR2] = np.append(
            ...
            create_column_vector(
                l2.pyrs[2].W_PI_lat[0], l2.pyrs[2].W_PI_lat[1], l2.pyrs[2].W_PI_lat[2]
            ),
            axis=1,
        )
        ...
        self._metrics[KEY_RULE_13_POST_DATA_L1] = np.append(
            ...
            create_column_vector(
                soma_act, basal_hat_act, post_soma_mp, post_basal_mp, post_val2
            ),
            axis=1,
        )
        self._metrics[KEY_RULE_13_WT_DATA_L1] = np.append(
            self._metrics[KEY_RULE_13_WT_DATA_L1], l2.pyrs[0].W_PP_ff[0]
        )
    ...
    def _train_1_step(
        self, nudge_predicate: bool
    ):  # Signature matched its abstract method b/c *args can be empty.
        """
        ...
        self.__train_1_step_rule_16b_and_rule_13(
            use_nudge=nudge_predicate
    ...
    def run(self):
        ...
        logger.info(
            f"Starting training {self.__self_prediction_steps} steps to 1b2b self predictive."
        )
        ...
        logger.info(
            f"Finished training {self.__after_training_steps} steps for p_exp 3b"
        )
